...
define('MID_TAXONOMY', $tax, true);


class Mid_Admin_Options {

public function __construct() {
    add_action( 'admin_menu', array( $this, '_midsettingsMenu' ) );
    add_action( 'admin_init', array( $this, '_midregisterSettings' ) );
    //add_action( 'admin_init', array( $this, '_midprocesmformActions' ) );
    
    if (isset($_GET['page']) && ( $_GET['page'] == 'mid-settings') ){
		add_action( 'admin_enqueue_scripts', array( $this, '_midAdminScripts' ) );
		add_action( 'admin_head', array( $this, '_midaddCustomScripts' ) );
	}

}
	
public function _middisplaySettings( $field, $meta = null, $repeatable = null ) {
	...
	$size = isset( $field['size'] ) ? $field['size'] : null;
  	...
  	$id = $name = isset( $field['id'] ) ? 'mid_options['.$field['id'].']' : null;
   ...
   switch($type) {
      ...
      case 'checkbox':
         ...
         echo '<input type="checkbox" value="'.($meta != '' ? 'true' : 'false').'" name="'.$name.'" id="'.$id.'" ',$meta != '' ? ' checked="checked"' : '',' />
         	<label for="mid_options['.$field['id'].']"><span class="description">'.$field['desc'].'</span></label>';
         echo '</div>';
      break;
      // value checkbox
		case 'value_checkbox':
      ...
      case 'radio':
         foreach ( $field['options'] as $option ) {
         	echo '<input type="radio" name="'.$name.'" id="mid_options['.$option['value'].']" value="'.$option['value'].'" ',$meta == $option['value'] ? ' checked="checked"' : '',' />
      ...
      case 'checkbox_group':
		 	foreach ($field['options'] as $option){
		 	    echo '<input type="checkbox" value="'.$option['value'].'" name="mid_options['.$field['id'].'][]" id="mid_options['.$option['value'].']"',$meta && in_array($option['value'], $meta) ? ' checked="checked"' : '',' />
		 	    		<label for="'.$option['value'].'">'.$option['label'].'</label><br />';
		 	}
         echo $desc;
      break;
      // tax_select
      case 'tax_select':
         echo '<select name="mid_options['.$field['id'].']" id="'.$id.'">
         		<option value="">-- '.__('Select', MID_LOCALE).' --</option>'; // Select One
         $terms = get_terms($field['id'], 'get=all');
         $selected = wp_get_object_terms('', 'mid_options['.$field['id'].']');
      ...
      // image
	  case 'image':
	      $image = MID_URL . '/images/usa-icon.png';	
	      echo '<div class="rg-mid_image" style="overflow:hidden;padding:0 0 0 10px">
	      		<span class="rg-mid_default_image" style="display:none">' . $image . '</span>';
	      if ( $meta ) { $image = wp_get_attachment_image_src( intval( $meta ), 'medium' ); }				
	      echo	'<input name="'.$name.'" type="hidden" class="rg-mid_upload_image" value="' . intval( $meta ) . '" style="width:'.$width.';" />
	      	     <img src="'.($meta ? $image[0] : $image).'" class="rg-mid_preview_image" style="display:block;max-width:300px;margin-bottom:10px;" />
	      		 <a href="#" class="rg-mid_upload_image_button button" rel="' . get_the_ID() . '" style="vertical-align:middle;">Choose Image</a>&nbsp;&nbsp;
	      		 <small style="vertical-align:middle;"><a href="#" class="rg-mid_clear_image_button">Remove Image</a></small>
	      	   </div>' . $desc;
	  break;
      // repeatable
      case 'repeatable':
		    ...
		    foreach( $meta as $value ):
		    	...
		    	echo '<li class="r-row">';
					...
					foreach($repeatable_fields as $repeatable_field):
					    ...
					    echo $this->_middisplaySettings( $repeatable_field, $meta[$i][$p++], array( $id, $i ) );
					endforeach;
		  ...
		  foreach( $meta as $value ):
		  	  ...
		  	  foreach($group_fields as $group_field ):
		  	    echo '<li class="r-row">';
		  	      ...
		  	      echo $this->_middisplaySettings( $group_field, $meta[$i][$p++], array( $id, $i ) );
		  	    echo '</li>';
		  	  endforeach;
		  	  $i++;
		  endforeach;
...
function _midfindFieldType( $needle, $haystack ) {
	foreach ( $haystack as $h )
		if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
			return $this->_midfindFieldType( $needle, $h['repeatable_fields'] );
		elseif ( isset( $h['type'] ) && $h['type'] == 'input_group' )
			return $this->_midfindFieldType( $needle, $h['group_fields'] );
		elseif ( ( isset( $h['type'] ) && $h['type'] == $needle ) || ( isset( $h['repeatable_type'] ) && $h['repeatable_type'] == $needle ) )
			return true;
	return false;
}

function _midfindRepeatableType( $needle = 'repeatable', $haystack ) {
	foreach ( $haystack as $h )
...
function _midvalidateThis($input) {
    $valid = array();
...
function _middisplayPage() {
   global $mid_fields;
   $meta = get_option( 'mid_options' );

   echo '<div class="wrap" id="rg-mf">';
      	echo '<div class="icon"></div>';
      	echo '<h2>' . __( 'Midway Options', MID_LOCALE ) . '</h2>';
      	
      	if ( ! isset( $_REQUEST['settings-updated'] ) )
   			$_REQUEST['settings-updated'] = false;
   			
   		if( $_GET['page'] == 'mid-settings' && isset($_GET['all-transients-deleted']) ):
		    echo '<div class="updated fade below-h2"><p>All Transients Deleted</p></div>';
		elseif( $_GET['page'] == 'mid-settings' && isset($_GET['transient-deleted']) ):
      ...
      echo '<div class="has-right-sidebar">';
      	...
      	echo '<div id="poststuff">';
      		// echo $this->_midsidebarMenu();
      	echo '</div>';
      
      	echo '<div style="float: left; width: 75%;" >';
      		echo '<form method="post" action="options.php" class="mid_options_form">';
      			settings_fields('mid_options');
      			wp_nonce_field( basename(__FILE__), 'mid_Admin_Page_Class_nonce' );
      			
      			echo '<div id="section_container">';
      			echo '<table class="form-table rg-mf">';
						foreach ( $mid_fields as $field ) {
						   if ( $field['type'] == 'section' ) {
						   	...
						   	echo '<tr class="' . $field['id'] . '">
						   			...
						   			echo $this->_middisplaySettings( $field, $meta[$field['id']] );
					...
					echo '</div>';      			
      			echo '<p class="submit"><input name="Submit" type="submit" class="button-primary" value="'. __( 'Save Options', MID_LOCALE ). '" /></p>';
      		echo '</form>';
      	echo '</div>';
...
public function _midsettingsMenu() {
   add_menu_page( 'Midway Options', 'Midway Options', 'edit_posts', 'mid-settings', array( $this, '_midsettingsOptions' ), 'dashicons-admin-generic' );
}


function _midsettingsOptions(){
	if (!current_user_can('manage_options')) {
	  wp_die( __('You do not have sufficient permissions to access this page.') );
	} else {
		$this->_middisplayPage();
	}
}
...
public function _midregisterSettings() {
	register_setting( 'mid_options', 'mid_options', array( $this, '_midvalidateThis' ) );
}
...
public function _categoryOrderGetSubCats($parentID){
  ...
  $subCatStr = "";
  $results=$wpdb->get_results("SELECT t.term_id, t.name FROM $wpdb->term_taxonomy tt, $wpdb->terms t, $wpdb->term_taxonomy tt2 WHERE tt.parent = $parentID AND tt.taxonomy = '".MID_TAXONOMY."' AND t.term_id = tt.term_id AND tt2.parent = tt.term_id GROUP BY t.term_id, t.name HAVING COUNT(*) > 0 ORDER BY t.term_order ASC");
...
public function _categoryOrderCatQuery($parentID){
  global $wpdb;
  return $wpdb->get_results("SELECT * FROM $wpdb->terms t inner join $wpdb->term_taxonomy tt on t.term_id = tt.term_id WHERE taxonomy = '".MID_TAXONOMY."' and parent = $parentID ORDER BY term_order ASC");
}
...
----------------------------------------- */
public function _midAdminScripts() {
	global $mid_fields;
	if( is_admin() && isset($_GET['page']) && ( $_GET['page'] == 'mid-settings' ) ) {
	
		$deps = array( 'jquery' );
		wp_enqueue_media();
		
		if ( $this->_midfindFieldType( 'spinnerslider', $mid_fields ) || $this->_midfindFieldType( 'slider', $mid_fields ) )
			wp_enqueue_script('jquery-ui-slider');
			wp_enqueue_script('spinner', MID_URL . '/js/spin.js', $deps, true);
			
		if ( $this->_midfindFieldType( 'repeatable', $mid_fields ) || $this->_midfindFieldType( 'input_group', $mid_fields ) )
			wp_enqueue_script('jquery-ui-sortable');
		
		wp_enqueue_script('thickbox', null, $deps );
		wp_enqueue_script('wp-color-picker');
		wp_enqueue_script('jquery-ui-datepicker');
		wp_enqueue_script('chosen', MID_URL . '/js/chosen.js', $deps, true );
		wp_enqueue_script('admin-js', MID_URL . '/js/custom-admin.js', $deps, true);
		
		wp_enqueue_style('thickbox');
		wp_enqueue_style('wp-color-picker');
		wp_enqueue_style('chosen-css', MID_URL. '/css/chosen.css' );
		wp_enqueue_style('admin-css', MID_URL . '/css/admin-panel.css');
		wp_enqueue_style('jquery-ui', '//ajax.googleapis.com/ajax/libs/jqueryui/1.8/themes/base/jquery-ui.css');		
	}
}  	

public function _midaddCustomScripts() {
   global $mid_fields;
