...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		// textarea
		case 'textarea':
			echo '<textarea name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" cols="' . esc_attr( $cols ) . '" rows="' . esc_attr( $rows ) . '" style="' . esc_attr( $width ) . '">' . esc_textarea( $meta ) . '</textarea>' . $desc;
		break;
		// editor
		case 'editor':
		...
		case 'color':
		    ...
		    jQuery("#' . $field['id'] . '").live("blur", function() { jQuery("#colorpicker-' . $field['id'] . '").slideUp(); } );
		    jQuery("#' . $field['id'] . '").live("focus", function() { jQuery("#colorpicker-' . $field['id'] . '").slideDown(); } );
		    });
		    </script>';
		  break;
		...
		case 'image':
			...
			echo '<input name="'.esc_attr( $name ).'" type="hidden" class="meta_box_upload_image" value="'.intval( $meta ).'" />
				  <img src="'.esc_attr( $image ).'" class="meta_box_preview_image" alt="'.$image->post_title.'" />
		...
		if ( in_array( get_post_type(), $this->page ) && ( meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) || meta_box_find_field_type( 'releasedate', $this->fields ) || meta_box_find_field_type( 'timepicker', $this->fields ) || meta_box_find_field_type( 'input_group', $this->fields ) || meta_box_find_field_type( 'repeatable', $this->fields ) ) ) {
			...
			foreach ( $this->fields as $field ) {
				switch( $field['type'] ) {
					...
					// tofromdate
					case 'input_group' :
						echo '$( "input#rg_tour_date_0_fromdate" ).datepicker({
							    defaultDate: "+1w",
							    dateFormat: "MM dd",
							    changeMonth: true,
							    numberOfMonths: 3,
							    onClose: function( selectedDate ) {
							      $( "input#rg_tour_date_0_todate" ).datepicker( "option", "minDate", selectedDate );
							    }
							  });
							  $( "input#rg_tour_date_0_todate" ).datepicker({
							    defaultDate: "+1w",
							    dateFormat: "MM dd",
							    changeMonth: true,
							    numberOfMonths: 3,
							    onClose: function( selectedDate ) {
							      $( "input#rg_tour_date_0_fromdate" ).datepicker( "option", "maxDate", selectedDate );
							    }
							  });';
					break;
					// slider
